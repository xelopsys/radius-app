...
const Sidebar: React.FC = () => {
	...
	return (
		<div className="w-10 md:w-[263px] h-full max-w-[263px] bg-[#3D639D] rounded-tr-3xl">
			<nav>
				<ul className="w-full h-max mt-10 flex flex-col justify-center items-end text-white stroke-white">
					{routes.map((route, index) => (
						<li
							key={index}
							className={`w-full md:w-[80%] text-sm rounded-l-3xl stroke-white hover:text-[#3D639D] hover:bg-white my-2 cursor-pointer ${
								pathname === route.path ||
						...
						>
							<Link href={route.path} className="w-full h-full ">
								<span className="w-full h-full flex flex-row justify-start items-center px-3 my-3">
									...
									<span className="mx-3 hidden md:inline"> {route.name}</span>
								</span>
							</Link>
						</li>
					))}
